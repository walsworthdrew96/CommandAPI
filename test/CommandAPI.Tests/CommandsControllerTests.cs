...
namespace CommandAPI.Tests
{
    public class CommandsControllerTests : IDisposable
    {
        ...
        {
            //Arrange
            //set up the mockRepo to use the contents of GetAllCommands(), return GetCommands(0) from the setup.
            mockRepo.Setup(repo => repo.GetAllCommands()).Returns(GetCommands(0));

            //set up mapper with a CommandsProfile instance ( the set of internal/external dto type conversions (maps) )
            var realProfile = new CommandsProfile();
            // make a MapperConfiguration for the CommandsProfile class 
            var configuration = new MapperConfiguration(cfg =>
            cfg.AddProfile(realProfile));
            // make a mapper object using that configuration
            IMapper mapper = new Mapper(configuration);

            //make a CommandsController using the mock repo and the mapper using the CommandsProfile maps
            var controller = new CommandsController(mockRepo.Object, mapper);
